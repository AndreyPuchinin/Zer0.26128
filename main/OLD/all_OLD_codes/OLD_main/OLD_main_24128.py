...
class Result(ZeroEntity):
    ...
    def __init__(self, swaps: RunningStrData, res_str: ResStrStruct, errors: Errors):
        """Устанавливает и хранит 3 поля: \n
           1. Свапы, \n
           2. 3 строки (класс ResStrStruct выше) \n
           3. ошибки"""
    ...
    def get_fields(self):
        """Вернет списком 3 поля: \n
           ...
           3. ошибки"""
        return [
            [one_swap.get_fields() for one_swap in self.swaps],
            self.res_str.get_fields(),
            self.errors.get_fields()
        ]


class RunningStrData(ZeroEntity):
    """Класс информации, коллекционирующейся при пробегании по строке \n
       ПОЛЯ (1): \n
       1. Информация. Тип - либо Свапы (список), либо тот же тип, что и сам объект (тоже список)"""

    def __init__(self, data: List[Union[OneSwap, List[RunningStrData]]]):
        """Устанавливает и хранит 3 поля: \n
           1. Информация. Тип - либо Свапы (список),
               либо список из подсписков из объектов того же типа (RunningStrData)"""
        self.data = data

    # Доперекинуть из Parser._collision_occurred(...)
    def check(self, sentence, words) -> List[OneSwap, RunningStrData]:
        pass
...
class Parser:
    ...
    @staticmethod
    def _suitable_val_entry_in_str(sentence: str, words_data_list: List[List[str | Card]]) -> \
            List[str | int | Card] | None:
        ...
        for char in sentence:
            ...
            # 3. На каждом шаге сортируем по длине от самой короткой до самой длинной все входные строки,
            #                начинающиеся на текущий символ.
            matched_words_data_list = sorted([data for data in words_data_list if data[1][0] == char],
    ...
    def _do_swaps(self, words: List[List[str | Card]]) -> RunningStrData:
        """ Замены (см. п. 4 ниже) \n
        \n
            Принимает на вход список из подсписков из \n
             ...
             -ссылки на карту. \n
        \n
            Возвращает RunningStrData \n
        \n
           1. Определяем результат функции по-умолчанию,  \n
               чтобы пучарм не ругался в конце функции  \n
           2. Пока есть вхождения (прерывание с условием внутри цикла) \n
           3. Получаем результат-структуру после поиска вхождений в строку значений нужного типа \n
           4. Условие прерывания (если больше НЕТ вхождений) \n
           5. Если вхождение есть...: \n
           5.1 Находим из найденной структуры выше \n
                имя, \n
                одно значение, подходящее по вхождению, \n
                соответствующую карту, \n
                и позицию вхождения значения \n
           5.2 Сохраняем последнее состояние строки \n
           5.3 Обновляем состояние строки: \n
               Заменяем в строке значение на имя \n
           5.4 Формируем текущий свап: \n
           ...
           5.5 Формируем переменную под все свапы класса \n
        ...
        while True:
            ...
            # 5. Если вхождение есть...:
                ...
                #      позицию замены,
                #      и соответствующую карточку
                one_cur_swap = OneSwap(last_version_of_str, self._str, found_link_val, found_link_name,
                                       found_pos, found_card)

                # 5.5 Формируем переменную под все свапы класса
                res_swaps = RunningStrData([one_cur_swap])

                # 5.6 Добавляем текущий сформированный свап в общий список свапов
                self._swaps += [res_swaps]
    ...
    def _simple_swaps(self) -> RunningStrData:
        """Прямые замены \n
        \n
           Ничего не принимает, \n
           Возвращает RunningStrData \n
        \n
           1. Создаем список под
               -имя карты,
               -значение нужного типа,
               -ссылку на карту.
           2. Формируем список из подсписков из \n
               -имени карты, \n
               -значения нужного типа, \n
               -ссылки на карту. \n
           3. Выполняем простые замены \n
        ...
        for card in self._cards:
            for val in card.usual_vals:
                words += [[card.name, val, card]]

        # 3. Выполняем простые замены
        #     и одним махом возвращаем информацию о присутствии замен
        return self._do_swaps(words)

    def _templates_swaps(self) -> RunningStrData:
    ...
    def _get_links_data(self, local_swaps: List[OneSwap]) -> dict:
        ...
        for one_swap in local_swaps[1:]:
            ...
            if one_swap.replacing in links_names.keys():
                links_names[one_swap.replacing] += [{one_swap.replaceable: 0}]

            # Если замещающее значение в текущем свапе
            #  совпадает с одним из уже найденных значений этой ссылки,
            #  добавляем более глубокое значение для данного значения данной ссылки
            # if one_swap.replacing in links_names[one_swap.replacing]:
    ...
    def _build_reverse_swap_seq(self, links_data: dict, templ: Template) -> List[List[str | int] | None]:
        """Производит последовательность обратных замен \n
        \n
           Принимает: \n
           1. Словарь с именами и значениями ссылок. Если код доехал до сюда, значит коллизий НЕТ!!! \n
           2. Шаблон из последней шаблонной замены \n
         \n
           Возвращает: \n
           Список из подсписков. Каждый подсписок:
           -то, что заменяем,
           -то, на что заменяем,
           -позиция каждой обратной замены (В РАМКАХ ШАБЛОНА!!! не строки!!!)
           При этом все обратные замены ранжированы последовательно
           по мере встречи ссылок в строке с лева на право!

           Если код доехал сюда, значит ни одной коллизии НЕ БЫЛО!!!

           1. Формируем переменную под первичную версию неотсортированных данных
               для обратных замен
           2. Перебираем все встречи ссылок в нетерминалах каждой из них
           3. Получаем:
              -то, что меняем (replaceable),
              -то, на что меняем (replacing),
              -позицию каждой замены (В РАМКАХ ШАБЛОНА!!! не строки!!!),
               и сохраняем их в первичную версию неотсортированных данных
               для обратных замен
           4. Формируем переменную под итоговую версию отсортированных данных
               для обратных замен
           5. Приступаем к сортировке первичной версии неотсортированных данных
               для обратных замен
           6. Возвращаем итоговую версию отсортированных данных
               для обратных замен"""
        ...
        # 2. Перебираем все встречи ссылок в нетерминалах каждой из них
        for one_link in templ.links:
            for one_pos in one_link.non_terminal.positions:
                # 3. Получаем:
                #    -то, что меняем (replaceable),
                #    -то, на что меняем (replacing),
                #    -позицию каждой замены (В РАМКАХ ШАБЛОНА!!! не строки!!!),
                #     и сохраняем их в первичную версию неотсортированных данных
                #     для обратных замен
                replaceable = one_link.non_terminal.the_link
                replacing = list(links_data[one_link.non_terminal.the_link])[0]
                unsorted_links_data += [[replaceable, replacing, one_pos]]
        ...
        # 5. Приступаем к сортировке первичной версии неотсортированных данных
        #     для обратных замен
        if unsorted_links_data:
            sorted_links_data = sorted(unsorted_links_data, key=lambda x: x[2])

        # 6. Возвращаем итоговую версию отсортированных данных
        #     для обратных замен
        return sorted_links_data

    def _do_all_reverse_swaps(self, links_data: dict, templ_swap: OneSwap) -> None:
        ...
        \n
         ...
         \n
           ...
           3. Получаем правильную последовательность всех обратных замен
              (то есть список из подсписков,
               каждый из которых хранит в себе всю информацию,
               нужную для каждой обратной замены. А именно:
               -то, что заменяем (replaceable),
               -то, на что заменяем (replacing),
               -и позицию каждой замены (pos)

              При этом каждый подсписок следует в правильной последовательности,
               то есть по мере удаления позиций каждой замены от левого конца строки)
           4. Находим вхождение последнего шаблона в строку
              (для того, чтобы "перешагнуть" через нужные символы,
               если они были перед шаблоном)
           5. Заводим переменную под смещение курсора позиции замены
           6. Наконец производим все обратные замены
           7. Получаем
              -то, что заменяем,
              -то, на что заменяем,
              -и позицию каждой замены. (В РАМКАХ ШАБЛОНА!!! Не строки!!!)
           8. Запоминаем последнюю версию строки для того,
               чтобы правильно сформировать обратный свап
           9. Производим-таки финальную замену!
           10. Добавляем обратный свап в поле свапов
           11. Смещаем курсор на разницу между замещаемым и замещающим"""
        ...
        #    При этом каждый подсписок следует в правильной последовательности,
        #     то есть по мере удаления позиций каждой замены от левого конца строки)
        reverse_swap_seq = self._build_reverse_swap_seq(links_data, last_templ)
        ...
        # 6. Наконец производим все обратные замены
        for one_swap in reverse_swap_seq:
            # 7. Получаем
            #    -то, что заменяем,
            #    -то, на что заменяем,
            #    -и позицию каждой замены. (В РАМКАХ ШАБЛОНА!!! Не строки!!!)
            replaceable, replacing, pos = one_swap

            # 8. Запоминаем последнюю версию строки для того,
            #     чтобы правильно сформировать обратный свап
            prev_str = self._str

            # print(self._str[:templ_pos + pos - pos_shift], '|', self._str[templ_pos + pos - pos_shift:])

            # 9. Производим-таки финальную замену!
            self._str = self._str[:templ_pos + pos - pos_shift] + replacing + \
                        self._str[templ_pos + pos + len(replaceable) - pos_shift:]

            # 10. Добавляем обратный свап в поле свапов
            self._swaps += [OneSwap(prev_str, self._str, replaceable, replacing, templ_pos + pos - pos_shift,
                                    templ_swap.card)]

            # 11. Смещаем курсор на разницу между замещаемым и замещающим
            pos_shift += len(replaceable) - len(replacing)

    def _prepare_for_reverse_swap(self, local_swaps: List[OneSwap]) -> None:
        ...
        if not self._collision_occurred(links_data):
            self._do_all_reverse_swaps(links_data, local_swaps[-1])

    def _reverse_swaps(self) -> RunningStrData:
        """Обратные замены \n
        \n
           Ничего не принимает, \n
           Возвращает RunningStrData \n
...
class Tests:
    ...
    def test_0(self):
        ...
        inp_str = '.1'
        user_str = 'user_str'
        usual_swaps = RunningStrData([])
        selfrefs_swaps = RunningStrData([])
        templ_swaps = RunningStrData([])
        reverse_swaps = RunningStrData([])
        all_swaps = RunningStrData([usual_swaps, selfrefs_swaps, templ_swaps, reverse_swaps])
        res_str = '.1'
        error = Errors()
        error.no_errors()
    ...
    def test_01(self):
        ...
        inp_str = '01234567'
        user_str = 'user_str'
        usual_swaps = [OneSwap('01234567', '0 один два 34567', '12', ' один два ', 1, card1)]
        selfrefs_swaps = []
        templ_swaps = []
        reverse_swaps = []
    ...
    def test_02(self):
        ...
        inp_str = '01234567'
        user_str = 'user_str'
        usual_swaps = [OneSwap('01234567', '012 три->семь ', '34567', ' три->семь ', 3, card1)]
        selfrefs_swaps = []
        templ_swaps = []
        reverse_swaps = []
        all_swaps = [usual_swaps, selfrefs_swaps, templ_swaps, reverse_swaps]
    ...
    def test_03(self):
        ...
        inp_str = '1312'
        user_str = 'user_str'
        usual_swaps = [OneSwap('1312', '13 один два ', '12', ' один два ', 2, card1)]
        selfrefs_swaps = []
        templ_swaps = []
        reverse_swaps = []
        all_swaps = [usual_swaps, selfrefs_swaps, templ_swaps, reverse_swaps]
    ...
    def test_1(self):
        ...
        inp_str = '.один'
        user_str = 'user_str'
        usual_swaps = [OneSwap('.один', '.число', 'один', 'число', 1, card1)]
        selfrefs_swaps = []
        templ_swaps = []
        reverse_swaps = []
        all_swaps = [usual_swaps, selfrefs_swaps, templ_swaps, reverse_swaps]
    ...
    def test_11(self):
        ...
        user_str = 'user_str'
        usual_swaps = [OneSwap('.один плюс один', '.число плюс один',
                               'один', 'число', 1, card1),
    ...
    def test_2(self):
        ...
        usual_swaps = [OneSwap('.один плюс один.', '.число плюс один.',
                       ...
                       OneSwap('.число плюс один.', '.число плюс число.',
                               'один', 'число', 12, card1)]
        selfrefs_swaps = []
        templ_swaps = [OneSwap('.число плюс число.', '.дважды число.',
                               'число плюс число', 'дважды число', 1, card2)]
        reverse_swaps = [OneSwap('.дважды число.', '.дважды один.',
                                 'число', 'один', 8, card2)]
        all_swaps = [usual_swaps, selfrefs_swaps, templ_swaps, reverse_swaps]
        res_str = '.дважды один.'
        error = Errors()
